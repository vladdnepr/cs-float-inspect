...
class Bot extends EventEmitter {
    ...
    constructor(settings, loginData) {
        ...
        this.auth = this.loginData.auth;

        this.userDataDir = `${this.settings.steam_user.dataDirectory}/${this.username}`;
        this.userRefreshTokenFile = `${this.userDataDir}/refresh_token.txt`;
    ...
    logIn() {
        ...
        let refreshToken = this.getSavedRefreshToken(); this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username; this.steamClient._name = this.username;

        if (refreshToken) {
            winston.info(`Refresh token found for ${this.username}. Logging on with refresh token.`);

            this.loginData = {
                refreshToken
            };
        } else {
            winston.info(`No refresh token saved for ${this.username}. Logging on with account name and password.`);
    ...
    bindEventHandlers()  { this.steamClient.on('debug', (err, r) => {winston.debug(this.username + ': ' + err);});
        ...
        this.steamClient.on('error', (err) => {
            ...
            if (errStr.includes('Proxy connection timed out')
                || errStr.includes('HTTP CONNECT 502 Bad Gateway')
            ) {
                this.logIn();
            }
    ...
    getSavedRefreshToken() {
        try {
            return fs.readFileSync(this.userRefreshTokenFile).toString('utf8').trim();
        } catch (ex) {
        }
    ...
    saveRefreshToken(token) {
        ...
        if (!utils.isValidDir(this.userDataDir)) {
            winston.info('Creating directory for ' + this.username);
            fs.mkdirSync(this.userDataDir);
        }

        fs.writeFileSync(this.userRefreshTokenFile, token,'utf8');
    }

    deleteSavedRefreshToken() {
        try {
            fs.unlinkSync(this.userRefreshTokenFile);
        } catch (ex) {
        }
    }
}
